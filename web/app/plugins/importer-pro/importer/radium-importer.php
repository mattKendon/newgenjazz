 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
	    ...
	    public function __construct() {
			...
			$this->slider 				= apply_filters('radium_theme_importer_slider_file', $this->demo_files_path . $this->slider_file_name);
    	...
    	public function intro_html() {
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
			    <p class="tie_message_hint">Importing demo data (Post, Pages, Images, Sliders, Widgets...) is the easiest way to setup your theme. It will
			    allow you to quickly edit everything instead of creating content from scratch. When you import the data following things will happen:</p>
			      ...
			      <ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
			          ...
			          <li>Posts, pages, main slider, some images, some widgets and menus will get imported .</li>
	     ...
	     * @return null
	     */
	    public function process_imports( $content = true, $widgets = true) {

			if ( $content && !empty( $this->content_demo ) && is_file( $this->content_demo ) ) {
				$this->set_demo_data( $this->content_demo );
			}

			if ( is_file( $this->content_demo ) ) {
				$this->set_demo_menus();
			}
			
			if ( class_exists( 'RevSlider' ) ) {
				if ( file_exists( $this->demo_files_path . $this->slider_file_name ) ) {	
				$newslider = new RevSlider();
				$newslider->importSliderFromPost( true, true, $this->demo_files_path . $this->slider_file_name );
				}
			}	
